...
const int r_i = 179;
const int ldr = A0;
const int ill_butt = 4;
int val_R = 0;
int val_G = 0;
int val_B = 0;
int avg[3] = {0,0,0};
bool illuminate = false;
int ill_butt_state = 0;
...
void setup() {
  ...
  pinMode(b, OUTPUT);
  pinMode(ldr, INPUT);
  pinMode(ill_butt, INPUT_PULLUP);
  Serial.begin(9600);
}

void loop() {
  if (digitalRead(ill_butt) == LOW){
    if (ill_butt_state == 0){
      ill_butt_state = 1;
    } else if(ill_butt_state == 1){
      ill_butt_state = 0;
    }
    delay(450);
  }
...
char* color_mapper(int mode, int R, int G, int B) { //passes color names for inputted RGB codes
  ...
  float plotbrown[3] = {abs(143-R), abs(98-G), abs(81-B)};
  avg_arr(plotbrown, brown);

  if (mode==0){
    float val_array[6]= {red, blue, green, cyan, magenta, yellow};
    min_finder(val_array, 6, minimum);
  }
  if (mode==1){
    float val_array[12]= {orange, kesari, Lgreen, saffron, pink, wheat, gold, turquoise, white, gray, teal, brown};
    min_finder(val_array, 12, minimum);
  }
